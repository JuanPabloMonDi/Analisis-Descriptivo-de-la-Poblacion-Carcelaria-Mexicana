...
#Funcion para recodificar algunas variables
recodificar_variable <- function(variable) {
  recodificada <- factor(variable, 
                         levels = c(1, 2, 8, 9), 
...
# Función para categorizar edades de consumo de tabaco
categorizar_consumo <- function(edad_vector) {
  # Identificar los valores mayores a 85 como "No consumió"
  categorias <- ifelse(edad_vector > 85, "No ha consumido", NA)
  
  # Crear una máscara para los valores válidos (<= 85)
  validos <- edad_vector[edad_vector <= 85]
  
  # Generar los cortes equitativos para los valores válidos
  cortes <- (quantile(validos, probs = seq(0, 1, length.out = 6), na.rm = TRUE))
  
  # Asignar las categorías a los valores válidos
  categorias[edad_vector <= 85] <- cut(edad_vector[edad_vector <= 85], 
                                       breaks = cortes, 
                                       include.lowest = TRUE, 
                                       labels = FALSE)
  
  # Convertir a factor y etiquetar los grupos
  categorias <- factor(categorias, 
                       levels = c("No ha consumido", 1:5),
                       labels = c("No ha consumido", paste0(round(cortes[[1]],3),"-",round(cortes[[2]],3)), paste0(round(cortes[[2]],3),"-",round(cortes[[3]],3)), paste0(round(cortes[[3]],3),"-",round(cortes[[4]],3)), paste0(round(cortes[[4]],3),"-",round(cortes[[5]],3)), paste0(round(cortes[[5]],3),"-",round(cortes[[6]],3))))
  
  return(categorias)
}

categorizar_tiempocond <- function(edad_vector) {
  # Identificar los valores mayores a 85 como "No consumió"
  categorias <- ifelse(edad_vector==0, "0", NA)
  
  # Crear una máscara para los valores válidos (<= 85)
  validos <- edad_vector[edad_vector>0]
  
  # Generar los cortes equitativos para los valores válidos
  cortes <- (quantile(validos, probs = seq(0, 1, length.out = 6), na.rm = TRUE))
  
  # Asignar las categorías a los valores válidos
  categorias[edad_vector > 0] <- cut(edad_vector[edad_vector > 0], 
                                       breaks = cortes, 
                                       include.lowest = TRUE, 
                                       labels = FALSE)
  ...
  categorias <- factor(categorias, 
                       levels = c("0", 1:5),
                       labels = c("0", paste0(round(cortes[[1]],3),"-",round(cortes[[2]],3)), paste0(round(cortes[[2]],3),"-",round(cortes[[3]],3)), paste0(round(cortes[[3]],3),"-",round(cortes[[4]],3)), paste0(round(cortes[[4]],3),"-",round(cortes[[5]],3)), paste0(round(cortes[[5]],3),"-",round(cortes[[6]],3))))
  
  return(categorias)
}

# Función para categorizar  cantidad de personas en la celda
categorizar_n_celda <- function(edad_vector) {
  # Identificar los valores mayores a 85 como "No consumió"
  categorias <- ifelse(edad_vector > 97, "No sabe/No responde", NA)
  
  # Crear una máscara para los valores válidos (<= 85)
  validos <- edad_vector[edad_vector <= 97]
  
  # Generar los cortes equitativos para los valores válidos
  cortes <- (quantile(validos, probs = seq(0, 1, length.out = 6), na.rm = TRUE))
  
  # Asignar las categorías a los valores válidos
  categorias[edad_vector <= 97] <- cut(edad_vector[edad_vector <= 97], 
                                       breaks = cortes, 
                                       include.lowest = TRUE, 
                                       labels = FALSE)
  ...
  # Convertir a factor y etiquetar los grupos
  categorias <- factor(categorias, 
                       levels = c("No sabe/No responde", 1:5),
                       labels = c("No sabe/No responde", paste0(cortes[[1]],"-",cortes[[2]]), paste0(cortes[[2]],"-",cortes[[3]]), paste0(cortes[[3]],"-",cortes[[4]]), paste0(cortes[[4]],"-",cortes[[5]]), paste0(cortes[[5]],"-",cortes[[6]])))
  
  return(categorias)
}

Tabla1<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_SOC.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla23<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_2_3.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla4<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_4.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla5<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_5.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla6<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_6.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla7<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_7.csv",locale = locale(encoding = "ISO-8859-1"))
Tabla89<- read_csv("Encuesta Carceles INEGI 2021/bd_enpol_2021_csv/ENPOL2021_8_9_10_11.csv",locale = locale(encoding = "ISO-8859-1"))
...
Tabla6 <- Tabla6[Variables6] %>%
  rename(
    ...
    CELDA_ALIMENTOS=as.factor(CELDA_ALIMENTOS)
  )
...
# Recodificamos algunas variables
Tabla1<-Tabla1 %>%
  mutate(across(c(16,17,18,19,20,21,23,25,27,29,31,33,35,37,39,41,43,45,
                  49,50,51,67,68,69,70,71,72,76,77,78,79,80,81,82,83,84,
                  86,87,88,89,90,91,92,93), recodificar_variable))


# REcodificamos el sexo
Tabla1$SEXO <- factor(Tabla1$SEXO, 
                      levels = c(1, 2), 
                      labels = c("M", "F"))

#discretizamos las personas en la celda
Tabla1$CAR_PERSONAS_CELDA<-categorizar_n_celda(Tabla1$CANTIDAD_PERSONAS_CELDA)

# Convertimos las variables de edad de consumo en categóricas
Tabla1 <- Tabla1 %>%
  mutate(across(.cols = c(22,24,26,28,30,32,34,36,38,40,42,44,46),
                .fns = ~ categorizar_consumo(.x),
                .names = "CATEGORIAS_{col}"))

Tabla1 <- Tabla1 %>%
  mutate(across(.cols = c(64,65),
                .fns = ~ categorizar_tiempocond(.x),
                .names = "CATEGORIAS_{col}"))
